...
//					myIp, myPort, pxClient->ulClientIP, pxClient->usClientPort ) );
		...
		case ECMD_STOR:
			...
			{
				/* In case an empty file was received ( bits1.bEmptyFile ), the transfer
				socket never delivered any data.  Check if the transfer socket is still open: */
				if( pxClient->xTransferSocket != FREERTOS_NO_SOCKET )
				{
					prvTransferCheck( pxClient );
				}
				...
				case ECMD_STOR:
					...
					if( pxClient->bits1.bEmptyFile != pdFALSE )
					{
						/* Although the 'xTransferSocket' is closed already, call this
						function just for the logging. */
						prvTransferCloseSocket( pxClient );
						/* Close an empty file. */
						prvTransferCloseFile( pxClient );
					}
...
static void prvTransferCloseSocket( xFTPClient *pxClient )
{
	...
	char pcStrBuf[ 32 ];
		...
		if( pxClient->bits1.bHadError == pdFALSE )
		{
	...
	if( pxClient->xTransferSocket != FREERTOS_NO_SOCKET )
	{
		...
		if( pxClient->ulRecvBytes == 0ul )
		{
			...
			pxClient->bits1.bEmptyFile = pdTRUE;
		}
		else
		{
			pxClient->bits1.bEmptyFile = pdFALSE;
	...
	pxClient->bits1.bIsListen = 0u;
	pxClient->bits1.bDirHasEntry = 0u;
	pxClient->bits1.bClientConnected = 0u;
	pxClient->bits1.bHadError = 0u;
}
/*-----------------------------------------------------------*/
...
/*-----------------------------------------------------------*/
	...
	BaseType_t xRc, xWritten;
		...
		UBaseType_t xStatus;
			...
			{
				/* The connection is not established (any more),
				therefore accept any amount of bytes, probably
				the last few bytes. */
			}
			else
			{
				if( xRc >= 512 )
				{
					/* More than a sector to write, round
					down to a multiple of 512 bytes. */
					xRc &= ~( 512 - 1 );
				}
				else
				{
				const struct xSTREAM_BUFFER *pxBuffer = FreeRTOS_get_rx_buf( pxClient->xTransferSocket );
					...
					if( pxBuffer->LENGTH - lTail >= 512 )
					{
						...
						but there is space for more. Just return and wait for more. */
						xRc = 0;
					}
					else
					{
...
BaseType_t xSetEvent = pdFALSE;
	...
	do
	{
		...
		xCount = FreeRTOS_min_int32( pxClient->xBytesLeft, xSpace );
		...
		if( ( pcBuffer != NULL ) && ( xBufferLength >= 512 ) )
		{
			...
			xCount = FreeRTOS_min_int32( xCount, xBufferLength );
			if( xCount > ( BaseType_t ) 0x40000 )
			{
				xCount = ( BaseType_t ) 0x40000;
			}
		}
		else
		{
			...
			pcBuffer = pcFILE_BUFFER;
			if( xCount > ( BaseType_t ) sizeof pcFILE_BUFFER )
			{
				xCount = ( BaseType_t ) sizeof pcFILE_BUFFER;
			}
		}

		if ( pxClient->xBytesLeft >= 1024 )
		{
			xCount &= ~( 512 - 1 );
		}
		...
		xItemsRead = ff_fread( pcBuffer, 1, xCount, pxClient->pxReadHandle );
		...
		xRc = FreeRTOS_send( pxClient->xTransferSocket, pcBuffer, xCount, 0 );
#endif /* ipconfigFTP_TX_ZERO_COPY */

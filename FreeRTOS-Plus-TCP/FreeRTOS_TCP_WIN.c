			...
			)
		{
			...
			if( pxSegment->u.bits.bAcked == pdFALSE )
			{
				if( xSequenceGreaterThan( pxSegment->ulSequenceNumber + ( uint32_t )ulDataLength, ulLast ) != pdFALSE )
				{
					...
					uint32_t ulFirst = pxSegment->ulSequenceNumber - pxWindow->tx.ulFirstSequenceNumber;
					FreeRTOS_debug_printf( ( "prvTCPWindowTxCheckAck[%u.%u]: %lu - %lu Partial sequence number %lu - %lu\n",
						...
						ulFirst, ulFirst + ulDataLength ) );

					/* Remove compiler warnings when FreeRTOS_debug_printf() is not
					defined. */
					( void ) ulFirst;
					break;
				}
